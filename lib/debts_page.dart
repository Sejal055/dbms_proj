...
class _DebtsPageState extends State<DebtsPage> {
  ...
  Future<void> _loadDebts() async {
    ...
    if (data != null && data['debts'] != null) {
      setState(() {
        _debts = List<Map<String, dynamic>>.from(data['debts'].map((d) {
          return {
  ...
  Future<void> _saveDebtsToFirestore() async {
    ...
    await _firestore.collection('users').doc(user.email).set({'debts': debtData}, SetOptions(merge: true));
  }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          ...
                          enabledBorder: OutlineInputBorder(
                            ...
                            borderSide: BorderSide(color: _currentDebtType == DebtType.lent ? incomeColor : expenseColor, width: 2),
                          ),
                          focusedBorder: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(12),
                            borderSide: BorderSide(color: _currentDebtType == DebtType.lent ? incomeColor : expenseColor, width: 3),
                          ),
                        ),
                      ),
                      const SizedBox(height: 20),
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          labelText: _currentDebtType == DebtType.lent ? 'Lent to (Person/Reason)' : 'Borrowed from (Person/Reason)',
                      ...
                      ListTile(
                        ...
                        leading: Icon(Icons.calendar_today, color: _currentDebtType == DebtType.lent ? incomeColor : expenseColor),
                        title: Text(
                          _selectedDueDate == null
                              ? 'Select Due Date (Optional)'
                              : 'Due Date: ${DateFormat('dd MMM yyyy').format(_selectedDueDate!)}',
                          style: TextStyle(color: _selectedDueDate == null ? bodyTextColor : headerTextColor),
                        ),
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton.icon(
                          ...
                          label: Text(
                            _currentDebtType == DebtType.lent ? "Record IOU (Lent)" : "Record Debt (Borrowed)",
                            style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white),
                          ),
                          style: ElevatedButton.styleFrom(
                            backgroundColor: _currentDebtType == DebtType.lent ? incomeColor : expenseColor,
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          title: const Text('Debts', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          leading: IconButton(
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isOverdue = dueDate != null && dueDate.isBefore(DateTime.now()) && status == DebtStatus.active;
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: isOverdue ? Colors.red.withOpacity(0.2) : Colors.black26.withOpacity(0.1),
                blurRadius: 8,
              ),
            ],
          ),
          child: Column(
            ...
            children: [
              ...
              Text(
                debt['person'],
                style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: headerTextColor),
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Text(
                        dueDate == null ? 'No Due Date' : 'Due: ${DateFormat('dd MMM yyyy').format(dueDate)}',
                        style: TextStyle(
                          fontSize: 13,
                          color: isOverdue ? Colors.red : bodyTextColor,
                  ...
                  if (status == DebtStatus.active)
                    TextButton.icon(
                      icon: const Icon(Icons.check_circle, size: 20, color: incomeColor),
