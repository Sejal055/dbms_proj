...
class _HistoryPageState extends State<HistoryPage> {
  ...
  Widget _buildTopSection(
      BuildContext context, Color primaryBlue, Color accentPurple) {
    return Container(
      ...
      builder: (context, snapshot) {
        ...
        for (var snap in snapshot.data!) {
          for (var doc in snap.docs) {
            ...
            final key =
                "${data['title'] ?? data['expense_name'] ?? ''}_${data['amount'] ?? data['expense_amount'] ?? 0}";
        ...
        final List<TransactionItem> transactions = allDocs.map((expense) {
          ...
          final String name =
              (data['title'] ?? data['expense_name'] ?? data['name'] ?? '')
                  .toString();
          final String category = (data['category'] ?? '').toString();

          // ⭐ FIX: Keep the full DateTime object
          DateTime? dateTime;
          ...
          // ⭐ FIX: Pass the dateTime object to the constructor
          return TransactionItem(
            icon: (type == 'Income' || type == 'Receive')
                ? Icons.account_balance_wallet
                : Icons.money_outlined,
            name: name,
            dateTime: dateTime, // <-- Pass the object
            category: category,
            amount: amount,
            ...
            return Column(
              ...
              children: [
                Padding(
                  ...
                  child: Text(
                    month, // This will now be "October 2025" etc.
                    style: const TextStyle(
                      fontSize: 24,
                      fontWeight: FontWeight.bold,
                  ...
                  Expanded(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // ⭐ FIX: Format the date/time here for display
                        Text(
                          transaction.dateTime != null
                              ? '${DateFormat('HH:mm').format(transaction.dateTime!)} - ${DateFormat('MMMM dd').format(transaction.dateTime!)}'
                              : 'No date',
                          style: TextStyle(
  ...
  // ⭐ FIX: Updated grouping function
  Map<String, List<TransactionItem>> groupTransactionsByMonth(
      List<TransactionItem> transactions) {
    final Map<String, List<TransactionItem>> grouped = {};
    for (var transaction in transactions) {
      // Format directly from the DateTime object.
      // If dateTime is null, group it under "Unknown".
      final String monthYear = transaction.dateTime != null
...
// ⭐ FIX: Updated TransactionItem class
class TransactionItem {
  final IconData icon;
  final String name;
  final DateTime? dateTime; // <-- Changed
  ...
  TransactionItem({
    required this.icon,
    required this.name,
    required this.dateTime, // <-- Changed
